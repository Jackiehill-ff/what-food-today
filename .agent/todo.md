...
- [x] 洁癖收尾（2026-09-18 第三轮后）：全量 JS/JSON 门禁通过、无真机禁忌语法（??/?.flatMap/catch{}）、文档与代码一致性核对无矛盾；当日三批改动（解析修复/UI 列表化/拖动优化）已提交并推送 origin/Mini-V1。开发版本 1.0.0 仍不含这些修复，真机验证前需重新上传。

- [x] 第三轮 UI（2026-09-18，模拟器实测）：编辑页食材/调味料列表长按拖动排序（类别内）；编辑弹窗去上下箭头、删除移右上角；菜单计划/采购清单拖动算法修复（拖动项 1:1 跟手 + 兄弟项按拖动项占位高度让位，消除乱跳/重叠）；采购「编辑采购项」弹窗去「分类」、删除/保存等长。
- [x] 六处 UI 微调（2026-09-18 第二轮，模拟器实测）：菜单计划空状态文案精简；「修改时间/删除」等宽挨着靠右；编辑页备注占位「无」；导入页占位「把你喜欢的食谱复制过来吧～」+ 解析键与方框间距；导入预览食材/调味料分组（添加按钮按类别插入）；编辑弹窗去掉「分类」栏。
- [x] 导入解析修复（2026-09-18）：「食材：」标签后逐行列食材的写法此前解析出 0 个食材；现支持同行/多行列表两种格式，新增「酱汁参考：」等自定义小节自动识别、全角「＋」分隔、「名称:用量」行、flomo 噪音行（链接/时间戳/#标签/分隔线）跳过；flomo 全量回归 124 条标题一致、19 条找回丢失食材、零垃圾项。
- [x] 编辑食谱页列表化改版（2026-09-18）：食材/调味料改为「名称+数量+编辑按钮」列表，单项编辑弹窗（名称/数量/分类，无单位栏，数量占位「无」），弹窗内删除/上移/下移；数量为空不显示「无」。
- [x] 菜单计划展开按钮改版（2026-09-18）：「⋮」方框按钮改为与食谱库一致的裸向下箭头（展开向上）。
- [x] 采购勾选弹窗顺序确认（2026-09-18）：按 `recipe.ingredients` 原始顺序渲染，与原食谱一致，未改代码；E2E 验证。
- [x] 模拟器 E2E（automator）：导入→解析→保存→编辑（列表/弹窗/分类切换/删除/排序）→菜单计划（添加/勾选弹窗顺序/箭头两态）全链路通过；测试菜谱「烤花菜暖沙拉」11 项食材 + 3 步做法。
