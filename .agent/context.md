...
- 导入中心：从 flomo 或其他文本来源粘贴食谱文本，解析成一个或多个草稿，预览编辑后保存到食谱库；也支持通过 `scripts/build-recipes-json.mjs` 批量生成 `recipes.json` 后从「数据 → 导入」导入；另支持上传图片识别文字（Tesseract.js 本地 OCR，中英文）填入文本框再解析。
- 食谱库：信息流优先，默认展示全部食谱卡片（菜名 + 食材名称）；轻点卡片展开完整做法与编辑/删除；编辑进入食谱编辑页（名称/分类/食材/调味料/做法/备注 + 食材排序），左上角返回箭头回信息流；支持搜索、分类筛选。
- 采购清单：统一的正式采购清单（不按日期分类，按分类排序），来自弹窗勾选和手动添加（按钮 + 弹窗）；支持复制清单文本。
