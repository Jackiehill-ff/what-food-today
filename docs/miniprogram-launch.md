...
- 已实现：菜单计划（日视图/加菜/弹窗勾选/改日期/删除/长按拖动排序/分享菜单：勾选当天菜谱生成图片，保存相册或发给朋友）、食谱库（信息流/搜索/分类/展开做法/加到菜单/编辑/删除）、食谱编辑（成品图上传移除/食材调味料添加置顶/排序）、采购清单（长按拖动排序/单项编辑删除/手动添加/批量删除/复制）、导入中心（粘贴文本解析，兼容「食材：/调味料：/做法：」标签与纯文本两种格式）、我的（微信一键登录/数据导出导入/统计）。
